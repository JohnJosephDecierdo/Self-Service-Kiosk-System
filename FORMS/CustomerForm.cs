...
namespace OOP_FINAL_PROJECT
{
    ...
    public partial class CustomerForm : Form
    {
        ...
        // Signals when background preload is complete
        private static readonly ManualResetEventSlim _cacheReady
        ...
        public CustomerForm()
        {
            ...
            WireIdleEvents();
            ShowIdle();
            // Preload images in background — menu will wait for cache before building cards
            if (!_cacheReady.IsSet)
                new Thread(PreloadImages) { IsBackground = true }.Start();
            // Reload menu instantly when Cook marks item unavailable
            SessionManager.MenuChanged += OnMenuChanged;
        }

        private void OnMenuChanged()
        {
            if (!IsDisposed && pnlOrdering.Visible)
                Invoke((Action)(() => LoadMenu("All")));
        }
        ...
        private void ShowIdle()
        {
            ...
            _lastOrderID = 0;
            _lastTotal   = 0;
            _currentUser = null;  // reset so next customer must log in again
            lblHeaderTitle.Text = "CUSTOMER — Kiosk";
        ...
        // Returns to idle for the SAME logged-in customer (no re-login on tap)
        private void ContinueOrdering()
        {
            ...
            _lastOrderID = 0;
            _lastTotal   = 0;
            // Keep _currentUser — TryLogin will skip login and go straight to menu
        ...
        private void StartCountdown()
        {
            ...
            _countdownTimer.Tick += (s, e) =>
            {
                _secondsLeft--;
                lblCountdown.Text = $"Screen resets in {_secondsLeft} seconds...";
                if (_secondsLeft <= 0) ContinueOrdering();
            };
        ...
        {
            // Wait up to 5 s for background preload; if still not done, proceed anyway
            _cacheReady.Wait(5000);

            foreach (Control c in flpMenu.Controls) c.Dispose();
            flpMenu.Controls.Clear();

            // Fetch ALL items (including unavailable) so we can show Sold Out cards
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                bool   available = row.Table.Columns.Contains("isAvailable")
                                   && row["isAvailable"] != DBNull.Value
                                   && Convert.ToBoolean(row["isAvailable"]);

                flpMenu.Controls.Add(available
                    ? BuildCard(itemID, name, price, cat)
                    : BuildSoldOutCard(name, cat));
            }
        ...
        private Panel BuildSoldOutCard(string name, string category)
        {
            // Use real food image just like normal cards
            _imageCache.TryGetValue(name, out Image cachedImg);
            ...
            card.Paint += (s, e) =>
            {
                ...
                using (var b = new SolidBrush(Color.FromArgb(180, 180, 180)))
                    g.FillRectangle(b, 0, 0, card.Width, 5);
                // Draw real image OR placeholder — same as normal card
                if (cachedImg != null)
                    g.DrawImage(cachedImg, new Rectangle(0, 5, card.Width, 92));
                else
                    DrawPlaceholder(g, category, new Rectangle(0, 5, card.Width, 92));
                // Gray overlay on image
                using (var overlay = new SolidBrush(Color.FromArgb(140, 200, 200, 200)))
                    g.FillRectangle(overlay, 0, 5, card.Width, 92);
                // SOLD OUT stamp
                using (var f = new Font("Segoe UI", 10f, FontStyle.Bold))
                using (var b2 = new SolidBrush(Color.FromArgb(200, 0, 0)))
                {
                    string lbl = "SOLD OUT";
                    var sz = g.MeasureString(lbl, f);
                    g.DrawString(lbl, f, b2,
                        (card.Width - sz.Width) / 2, 5 + (92 - sz.Height) / 2);
                }
            };
            ...
            {
                Text      = name,
                Left      = 4, Top = 100, Width = 147, Height = 36,
                Font      = new Font("Segoe UI", 8.5f, FontStyle.Bold),
                TextAlign = ContentAlignment.MiddleCenter,
            ...
            {
                Text      = "Not Available",
                Left      = 4, Top = 136, Width = 147, Height = 20,
                Font      = new Font("Segoe UI", 8f, FontStyle.Italic),
                TextAlign = ContentAlignment.MiddleCenter,
                ForeColor = Color.FromArgb(200, 0, 0),
                BackColor = Color.Transparent,
            };
            var bDisabled = new Button
            {
                Text      = "Sold Out",
                Left = 8, Top = 161, Width = 139, Height = 30,
            ...
            bDisabled.FlatAppearance.BorderSize = 0;

            card.Controls.AddRange(new Control[] { lName, lStatus, bDisabled });
            return card;
        }
        ...
        {
            int selectedIndex = dgvCart.SelectedRows.Count > 0
                ? dgvCart.SelectedRows[0].Index : -1;
        ...
        {
            if (!Directory.Exists(_imgDir)) { _cacheReady.Set(); return; }
            ...
            _cacheReady.Set(); // signal that all images are ready
        }
