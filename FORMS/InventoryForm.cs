    ...
    public partial class InventoryForm : Form
    {
        ...
        private void ColorInventoryRows()
        {
            ...
            foreach (DataGridViewRow row in dgvInventory.Rows)
            {
                ...
                if (cond == "Damaged & Lost")
                    row.DefaultCellStyle.BackColor = Color.FromArgb(255, 200, 150); // orange-red
                else if (cond == "Lost")
                    row.DefaultCellStyle.BackColor = Color.FromArgb(255, 220, 220); // red
                else if (cond == "Damaged")
                    row.DefaultCellStyle.BackColor = Color.FromArgb(255, 235, 180); // yellow
                else if (qty <= min)
                    row.DefaultCellStyle.BackColor = Color.FromArgb(255, 248, 220); // pale yellow
                else
        ...
        private void PopulateForm(DataGridViewRow row)
        {
            ...
            // Clamp to valid range so corrupted negative quantities don't crash the spinner
            decimal qty = Convert.ToDecimal(row.Cells["quantity"]?.Value ?? 0);
            nudQuantity.Value = Math.Max(nudQuantity.Minimum, Math.Min(nudQuantity.Maximum, qty));

            decimal min = Convert.ToDecimal(row.Cells["minStock"]?.Value ?? 0);
            nudMinStock.Value = Math.Max(nudMinStock.Minimum, Math.Min(nudMinStock.Maximum, min));
        ...
        private void btnRestoreCondition_Click(object sender, EventArgs e)
        {
            ...
            int maxRestorable = _repo.GetRestorableQuantity(id);
            if (maxRestorable <= 0)
            {
                MessageBox.Show(
                    $"'{name}' has no unrestored damaged or lost quantity to recover.",
                    "Nothing to Restore",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            using (var dlg = new InventoryActionDialog("Restore", name, maxRestorable, condition))
            {
                if (dlg.ShowDialog() != DialogResult.OK) return;
                if (_repo.RestoreCondition(id, dlg.Quantity, dlg.Remarks, _managerUsername, dlg.RestoreType))
                {
                    MessageBox.Show($"'{name}' restore recorded.", "Restored",
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                    LoadInventory();
                    LoadAllLogs();
                }
        ...
        {
            // For actions that subtract from quantity, cap the spinner at current stock
            int maxQty = 9999;
            if (action == "Check Out" || action == "Damaged" || action == "Lost")
            {
                maxQty = _repo.GetCurrentQuantity(itemID);
                if (maxQty <= 0)
                {
                    MessageBox.Show($"'{itemName}' has no available quantity to {action.ToLower()}.",
                        "Insufficient Stock", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }

            using (var dlg = new InventoryActionDialog(action, itemName, maxQty))
            {
                if (dlg.ShowDialog() != DialogResult.OK) return;
                ...
                switch (action)
                {
                    ...
                    case "Check Out":
                        ok = _repo.CheckOut(itemID, qty, remarks, _managerUsername);
                        if (!ok) { MessageBox.Show("Not enough stock to check out!", "Insufficient Stock",
                            MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
                        break;
                    case "Damaged":
                        ok = _repo.ReportDamage(itemID, qty, remarks, _managerUsername);
                        if (!ok) { MessageBox.Show("Not enough stock to mark as damaged!", "Insufficient Stock",
                            MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
                        break;
                    case "Lost":
                        ok = _repo.ReportLoss(itemID, qty, remarks, _managerUsername);
                        if (!ok) { MessageBox.Show("Not enough stock to mark as lost!", "Insufficient Stock",
                            MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
                        break;
                }
